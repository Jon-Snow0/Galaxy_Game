...
AudioSample boom;//explosion sound
AudioSample pew;//bullet shooting sound
...
void generateBalls() {//initial generation of asteroids 
  for (int i = 0; i<150; i++)
  {
    ball[i]=loadImage("asteroid6.png");//loads 150 of the asteroid images
...
void intro1()//controls music, looping for the ship, ship movement,
             //bullet generatioa and movement, check for collisions, 
...
void setup() {
  ...
  {
    String name1 ="spaceship"+i+".png";
    images1[i] = loadImage(name1);
    images1[i].resize(shipW, shipH);
  }
  back1 = loadImage("space1.jpg"); 
  back2 = loadImage("space2.jpg"); 
  ...
  player=minim.loadFile("gamemusic.mp3");
